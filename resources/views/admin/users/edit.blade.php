...
@php
    ...
    $selected = $user->isMaster() ? [] : $user->permissionList();
@endphp
...
<div class="max-w-4xl">
    ...
    <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="space-y-6"
        x-data="permissionPicker({{ Js::from($selected) }}, {{ Js::from($moduleSpec) }}, {{ Js::from($user->role) }})">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" required placeholder="Enter email"
                        class="w-full rounded-xl border border-gray-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition">
                </div>
                <div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6" x-show="role === 'master_admin'" x-cloak>
            <div class="flex items-start gap-3 bg-violet-50 border border-violet-200 rounded-xl p-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-violet-600 mt-0.5">This account automatically has full access to every module. No permissions need to be selected.</p>
                </div>
            </div>
        </div>

        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
            @include('admin.users._permissions', ['selected' => $selected])
        </div>
